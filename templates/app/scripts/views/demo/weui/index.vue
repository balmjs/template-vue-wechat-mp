<template>
  <div>
    ...
    <div class="cnt">
      ...
      <wx-view class="item">
        ...
        <div class="comp-cnt">
          <wx-button
            ...
            >显示 mp-gallery</wx-button
          >
          <mp-gallery
            :show="mpGallery.show.toString()"
            @change="log('[mp-gallery] change', $event.detail)"
            @delete="log('[mp-gallery] delete', $event.detail)"
            @hide="mpGallery.show = false"
            :img-urls="mpGallery.imgUrls"
            delete="true"
            hide-on-click="true"
            current="1"
          ></mp-gallery>
      ...
      <wx-view class="item">
        ...
        <div class="comp-cnt">
          <div class="weui-grids">
            <div
              class="weui-grid mp-icon-cnt"
              v-for="item in mpIcon.list"
              :key="item.icon"
            >
              <mp-icon
                :type="mpIcon.filled ? 'filled' : 'outline'"
                :icon="item.icon"
                :color="item.color"
                :size="item.size"
              ></mp-icon>
          ...
          <div class="opr-cnt">
            ...
            <wx-switch
              @change="mpIcon.filled = $event.detail.value"
            ></wx-switch>
          </div>
        </div>
      </wx-view>
      <wx-view class="item">
        ...
        <div class="comp-cnt">
          <mp-cells title="带说明的列表项">
            ...
            <mp-slideview
              show="true"
              :buttons="mpSlideView.buttons"
              @buttontap="log('[mp-slideview] buttontap', $event.detail)"
            >
      ...
      <wx-view class="item">
        <div class="title">
          mp-form/mp-form-page/mp-checkbox-group/mp-checkbox
        </div>
            ...
            <div slot="tips">
              <label class="weui-agree">
                <input
                  ...
                  @change="mpForm.isAgree = !!$event.currentTarget.checked"
                />
                <wx-text
                  class="weui-agree__checkbox"
                  :class="mpForm.isAgree ? 'checked' : ''"
                ></wx-text>
      ...
      <wx-view class="item">
        ...
        <div class="comp-cnt">
          ...
          <mp-dialog
            title="test"
            :show="mpDialog.dialogShow"
            @buttontap="mpDialog.dialogShow = false"
            :buttons="mpDialog.buttons"
          >
            <view>test content</view>
          </mp-dialog>
          <mp-dialog
            title="test1"
            :show="mpDialog.showOneButtonDialog"
            @buttontap="mpDialog.showOneButtonDialog = false"
            :buttons="mpDialog.oneButton"
          >
        ...
        <div class="comp-cnt">
          ...
          <div class="opr-cnt">
            <input
              class="weui-input"
              placeholder="输入文本"
              v-model="mpToptips.value"
            />
          </div>
          <wx-button class="wx-button" type="default" @tap="showMpToptips"
            >确认</wx-button
          >
        </div>
      </wx-view>
      <wx-view class="item">
        ...
        <div class="comp-cnt">
          ...
          <mp-half-screen-dialog
            :show="mpHalfScreenDialog.typeF"
            @close="mpHalfScreenDialog.typeF = false"
          >
            <div slot="title">测试标题A</div>
            <div slot="desc">这是自定义描述区</div>
            <div slot="footer">
              <wx-button type="default" class="weui-btn weui-btn_default"
                >按钮A</wx-button
              >
              <wx-button type="primary" class="weui-btn weui-btn_primary"
                >按钮B</wx-button
              >
            </div>
          </mp-half-screen-dialog>
          ...
          ></mp-half-screen-dialog>
          <mp-half-screen-dialog
            :show="mpHalfScreenDialog.typeT"
            closabled="false"
            @close="mpHalfScreenDialog.typeT = false"
          >
            <div slot="title">测试标题A</div>
            <div slot="desc">这是自定义描述区</div>
            <div slot="footer">
      ...
      <wx-view class="item">
        ...
        <div class="comp-cnt">
          ...
          <mp-actionsheet
            ...
            @actiontap="log('[mp-actionsheet] action tap', $event.detail)"
          ></mp-actionsheet>
        </div>
      </wx-view>
      <wx-view class="item">
        ...
        <div class="comp-cnt">
          ...
          <div class="opr-cnt">
            <div class="opr-label">
              ...
              <wx-switch
                @change="mpNavigationBar.loading = $event.detail.value"
              ></wx-switch>
            </div>
            <wx-button
              class="wx-button opr-button"
            ...
            <div class="opr-cnt">
              ...
              <wx-switch
                @change="mpNavigationBar.show = $event.detail.value"
              ></wx-switch>
            </div>
            <div class="opr-cnt">
              <div class="opr-label">动画</div>
              <wx-switch
                @change="mpNavigationBar.animated = $event.detail.value"
              ></wx-switch>
            </div>
          </div>
        </div>
      ...
      <wx-view class="item">
        ...
        <div class="comp-cnt">
          <mp-tabbar
            ...
            "
            :list="mpTabbar.list"
            @change="log('[mp-tabbar] change', $event.detail)"
          ></mp-tabbar>
          <div class="opr-cnt">
            <div class="opr-label">显示/隐藏</div>
            <wx-switch
              @change="mpTabbar.show = $event.detail.value"
            ></wx-switch>
          </div>
        </div>
      </wx-view>
      <wx-view class="item" style="padding-bottom: 150px">
        ...
        <div class="comp-cnt">
          <mp-searchbar
            @selectresult="log('[mp-searchbar] selectresult', $event.detail)"
            :search="onSearch"
          ></mp-searchbar>
        </div>
      </wx-view>
      <wx-view class="item">
        ...
        <div class="comp-cnt" style="padding-bottom: 60px">
          <mp-grids
            :grids="mpGrids.grids"
            @gridtap="log('[mp-grids] tap', $event.detail)"
          ></mp-grids>
        </div>
      </wx-view>
    </div>
    ...
    return {
      ...
      mpIcon: {
        ...
        list: [
          ...
          { icon: 'add', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'add2', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'album', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'arrow', color: 'rgba(0, 0, 0, .9)', size: 12, name: '' },
          { icon: 'at', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'back', color: 'rgba(0, 0, 0, .9)', size: 12, name: '' },
          { icon: 'back2', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          {
            icon: 'bellring-off',
            color: 'rgba(0, 0, 0, .9)',
            size: 25,
          ...
          { icon: 'camera', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'cellphone', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'clip', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'close', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'close2', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'comment', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'contacts', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'copy', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'delete-on', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'delete', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'discover', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'display', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'done', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'done2', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'download', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'email', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'error', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'eyes-off', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'eyes-on', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'folder', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          {
            icon: 'group-detail',
            color: 'rgba(0, 0, 0, .9)',
            size: 25,
          ...
          { icon: 'help', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'home', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'imac', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'info', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'keyboard', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'like', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'link', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'location', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'lock', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          {
            icon: 'max-window',
            color: 'rgba(0, 0, 0, .9)',
            size: 25,
          ...
          { icon: 'me', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'mike', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'mike2', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          {
          ...
          { icon: 'more', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'more2', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'mosaic', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'music-off', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'music', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'note', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'pad', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'pause', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'pencil', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          {
            icon: 'photo-wall',
            color: 'rgba(0, 0, 0, .9)',
            size: 25,
          ...
          { icon: 'play', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'play2', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'previous', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'previous2', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'qr-code', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'refresh', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          {
            icon: 'report-problem',
            color: 'rgba(0, 0, 0, .9)',
            size: 25,
          ...
          { icon: 'search', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'sending', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'setting', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'share', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'shop', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'star', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'sticker', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'tag', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'text', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'time', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          {
            icon: 'transfer-text',
            color: 'rgba(0, 0, 0, .9)',
            size: 25,
          ...
          { icon: 'transfer2', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'translate', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          { icon: 'tv', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          {
            icon: 'video-call',
            color: 'rgba(0, 0, 0, .9)',
            size: 25,
          ...
          { icon: 'voice', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' },
          {
            icon: 'volume-down',
            color: 'rgba(0, 0, 0, .9)',
            size: 25,
          ...
          { icon: 'volume-up', color: 'rgba(0, 0, 0, .9)', size: 25, name: '' }
        ]
      },
      mpCells: {
